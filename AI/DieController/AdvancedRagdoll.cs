...
{
    public bool copyTransformFromBoneName = false;
    ...
    BoneNamesForCharacters[] allBoneNames;
    ...
    public void SynchRagdollIn(Transform src)
    {
        ...
        if (copyTransformFromBoneName)
        {
            CopyTransforms(src, transform);
        }
        else
        {
            allBoneNames = src.GetComponentsInChildren<BoneNamesForCharacters>();

            CopyTransformByEnumName(src, transform);
        }
    }
